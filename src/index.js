...
const { v4: uuidv4, validate: uuidIsValid } = require('uuid');
...
function checksCreateTodosUserAvailability(request, response, next) {
    const { user } = request;
    const maxLimitOfTodos = user.todos.length >= 10;

    if (!user.pro && maxLimitOfTodos) {
        return response
            .status(403)
            .json({ error: 'Maximum limit of todos for the Free plan' });
    }
